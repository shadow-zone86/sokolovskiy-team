...
/**
 * Виджет: подвал сайта — копирайт, ссылки на контакт.
 */
import { siteContent } from '@/shared/config/siteContent'

const { footer, contacts } = siteContent
const currentYear = new Date().getFullYear()
</script>

<template>
  <footer class="visit-footer" role="contentinfo">
    <div class="visit-footer__inner">
      <p class="visit-footer__copyright">
      ...
      <div class="visit-footer__links">
        ...
        <a
          ...
          class="visit-footer__link visit-footer__link--icon"
          :aria-label="`Telegram: ${contacts.telegramLabel}`"
        >
          <span class="visit-footer__telegram-icon" aria-hidden="true">
...
.visit-footer__links {
  @include flex-row($spacing-4, center, flex-end);
}
...
.visit-footer__link {
  ...
  &--icon {
    @include flex-row($spacing-1, center, center);
  }
}

.visit-footer__telegram-icon {
  display: inline-flex;
  line-height: 0;
